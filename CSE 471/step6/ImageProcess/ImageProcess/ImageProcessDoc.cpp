...
BEGIN_MESSAGE_MAP(CImageProcessDoc, CDocument)
	...
	ON_COMMAND(ID_GENERATE_FILLGREEN, &CImageProcessDoc::OnGenerateFillgreen)
	...
	ON_COMMAND(ID_FILTER_3X3MEDIAN, &CImageProcessDoc::OnFilter3x3median)
END_MESSAGE_MAP()
...
void CImageProcessDoc::OnGenerateFillgreen()
{
	//MIstake
}
...
void CImageProcessDoc::OnFilterMonochrome()
{
	...
	for (int r = 0; r < m_image1.GetHeight(); r++)
	{
		for (int c = 0; c < m_image2.GetWidth() * 3; c++)
		{
			auto mono = (m_image1[r][c * 3 + 0] + m_image1[r][c * 3 + 1] + m_image1[r][c * 3 + 2]) / 3;

			m_image2[r][c * 3 + 0] = BYTE(mono);       // Blue
			m_image2[r][c * 3 + 1] = BYTE(mono);   // Green
			m_image2[r][c * 3 + 2] = BYTE(mono);   // Red
...
void CImageProcessDoc::OnFilter3x3median()
{
	// Allocates an output image the same size as the input image
	m_image2.SetSameSize(m_image1);
		...
		for (int c = 0; c<m_image2.GetWidth() * 3; c++)
		{
			...
			for (int i = -1; i <= 1; i++)
			{
				if ((r + i) >= 0 && (r + i) < m_image2.GetHeight())
				{
					...
					{
						if ((c + j) >= 0 && (c + j) < m_image2.GetWidth() * 3)
							pixels.push_back(m_image1[r + i][c + j]);
					}
				}
			}
			...
			if (size % 2 == 0)
			{
				middle = (pixels[size / 2] + pixels[(size / 2) - 1] / 2);
			}
			else
			{
				middle = pixels[size / 2];
			}
			m_image2[r][c] = BYTE(middle);
		}
	}
