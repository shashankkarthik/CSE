...
def type_check_types(type_a, type_b):
    if type_a != type_b:
        print("Type Mismatch in Assignment ({} != {})".format(type_a, type_b))
        exit(1)
...
class FunctionCallNode(Node):
    ...
    def backup_variables(self, ic_instructions, entries):
        ...
        if not in_function:
            return
        variables = list(symbol_table.SYMBOL_TABLE.active_entries)
    ...
    def generate_ic(self, ic_instructions):
        for child in self.children:
            ...
            if type_ in symbol_table.SCALAR_TYPES:
                ...
                print("Print doesn't know how to handle this type ({})".format(
                    type_))
                exit(1)
        ...
        for i, letter in enumerate(letters):
            ...
            if escaped:
                if letter in {'"', '\\', 'n', 't'}:
                    ...
                    print("Unknown Escape character after slash ({})".format(
                        letter))
                    exit(1)
            escaped_letters.append(letter)
...
class IndexingNode(Node):
    ...
    def __init__(self, array_id, index_expression):
        ...
        if type_ not in symbol_table.ARRAY_TYPES:
            print("Can't Index Into Non-Array Type ({})".format(type_))
            exit(1)
        if index_expression.expression_type != "val":
            print("Must use val to index into expression")
            exit(1)
        if type_ == "array(val)":
            scalar_type = "val"
        else:
...
class ArraySizeNode(Node):
    ...
    def __init__(self, array_id):
        ...
        type_ = entry.expression_type
        if type_ not in symbol_table.ARRAY_TYPES:
            print("Can't Size A Non-Array Type ({})".format(type_))
            exit(1)
...
class ArrayResizeNode(Node):
    ...
    def __init__(self, array_id, expression):
        ...
        type_ = entry.expression_type
        if type_ not in symbol_table.ARRAY_TYPES:
            print("Can't Resize A Non-Array Type ({})".format(type_))
            exit(1)
