...
def p_array__methods(p):
    """
    expression : ID '.' ID '(' empty_or_more_comma_sep_expr ')'
    """
    object_ = p[1]
    method = p[3]
    children = p[5].children
    if method == 'size':
        ...
        p[0] = ast.ArrayResizeNode(p[1], expression)
    else:
...
def parse(filename=None, debug_mode=False):
    ...
    parse_tree = parser.parse(source, lexer=lexer.lexer)
