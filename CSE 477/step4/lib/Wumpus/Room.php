...
class Room {
    ...
    public function contains($item, $recurse=0) {
        ...
        //Check in current room and it's neighbors 1 room away
        elseif ($recurse == 1) {
            if ($this->contains($item, $recurse-1)) {
                return True;
            }
            else {
        ...
        //Check in current room and it's neighbors up to 2 rooms away
        elseif ($recurse == 2) {
            if ($this->contains($item, $recurse-1)) {
                return True;
            }
            else {
                foreach ($this->neighbors as $firstNeighbor) {
                    foreach ($firstNeighbor->neighbors as &$secondNeighbor) {
                        if (in_array($item, $secondNeighbor->contents)) {
                            return True;
                        }
                    }
                    unset($secondNeighbor);
                }
                unset($firstNeighbor);
